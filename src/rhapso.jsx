...
// ─── Font Injection ───────────────────────────────────────────────────────────
if (typeof document !== "undefined" && !document.getElementById("rhapso-fonts")) {
...
// Overlay zones (percentage-based on SVG mannequin container)
const ZONES = {
...
const STORE = "rhapso_v1";
const persist = (d) => { try { localStorage.setItem(STORE, JSON.stringify(d)); } catch {} };
const hydrate = () => { try { return JSON.parse(localStorage.getItem(STORE)); } catch { return null; } };
...
// ─── Mannequin SVG (2D editorial silhouette) ──────────────────────────────────
function Mannequin({ skinHex = "#FDDBB4", bodyId = "standard" }) {
  ...
  const tx = (w) => `matrix(${w},0,0,1,${100*(1-w)},0)`;
  ...
  return (
    <svg viewBox="0 0 200 500" xmlns="http://www.w3.org/2000/svg"
      ...
      {/* ── TORSO + ARMS (scaled together) ── */}
      <g transform={tx(torsoW)}>
        {/* Torso body */}
        <path d="M62,73 Q37,83 35,114 L35,186 Q35,198 50,200 L150,200 Q165,198 165,186 L165,114 Q163,83 138,73 Q120,68 100,68 Q80,68 62,73Z"
              fill={skinHex} />
        {/* Waist indent hint */}
...
// ─── Mannequin + Garment Overlays ─────────────────────────────────────────────
function MannequinCanvas({ mannequin, garments, onZoneClick, onGarmentClick }) {
  ...
  return (
    <div style={{ position: "relative", width: "100%", userSelect: "none" }}>
      ...
      {/* Garment overlays + zone buttons (top, bottom, shoes) */}
          ...
          <div key={cat.id} style={{ position: "absolute", ...zone, display: "flex", alignItems: "center", justifyContent: "center" }}>
            {g ? (
              <div style={{ position: "relative", width: "100%", height: "100%", cursor: "pointer" }}
                   onClick={() => onGarmentClick(g)} title={`Comprar: ${g.name}`}>
                <img
                  src={g.imageUrl}
                  alt={g.name}
                  style={{ width: "100%", height: "100%", objectFit: "contain", filter: "drop-shadow(0 4px 12px rgba(0,0,0,0.18))" }}
                />
              </div>
            ) : (
              <button className="zone-btn"
                ...
                style={{
                  ...
                  border: "1px dashed rgba(180,165,140,0.5)", borderRadius: 4,
                  color: C.muted, fontSize: 11, fontFamily: "'Jost',sans-serif",
      ...
      {/* Accessory — floats to the right side */}
      <div style={{ position: "absolute", top: "26%", right: "-22%", width: "30%", height: "28%", display: "flex", alignItems: "center", justifyContent: "center" }}>
        {garments.accessory ? (
          <div style={{ position: "relative", width: "100%", height: "100%", cursor: "pointer" }}
               onClick={() => onGarmentClick(garments.accessory)} title="Comprar acessório">
            <img src={garments.accessory.imageUrl} alt={garments.accessory.name}
                 style={{ width: "100%", height: "100%", objectFit: "contain", filter: "drop-shadow(0 3px 10px rgba(0,0,0,0.16))" }} />
          </div>
        ) : (
          <button className="zone-btn"
            ...
            style={{
              width: "100%", height: "100%", background: "rgba(255,255,255,0.2)",
              border: "1px dashed rgba(180,165,140,0.5)", borderRadius: 4,
              color: C.muted, fontSize: 10, fontFamily: "'Jost',sans-serif",
...
// ─── Home Screen ──────────────────────────────────────────────────────────────
function HomeScreen({ looks, onNew, onOpen, onDelete }) {
  return (
    <div className="fadeIn" style={{ minHeight: "100vh", background: C.bg }}>
      {/* Header */}
      <header style={{
        borderBottom: `1px solid ${C.border}`, padding: "24px 48px",
        display: "flex", alignItems: "center", justifyContent: "space-between",
        background: C.surface,
      }}>
        <div>
          <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 30, fontWeight: 300, letterSpacing: "0.2em", color: C.text }}>
            RHAPSO
          </div>
          <div style={{ fontSize: 10, fontWeight: 300, letterSpacing: "0.25em", color: C.muted, textTransform: "uppercase", marginTop: 2 }}>
            Virtual Wardrobe
          </div>
        </div>
        <button className="pill-btn"
          onClick={onNew}
          style={{
            padding: "10px 28px", background: C.text, color: C.white,
            fontSize: 11, fontWeight: 400, letterSpacing: "0.12em",
            textTransform: "uppercase", border: `1px solid ${C.text}`,
        ...
        {looks.length === 0 ? (
          ...
          <>
            <div style={{ display: "flex", alignItems: "baseline", gap: 16, marginBottom: 40 }}>
              <h2 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 26, fontWeight: 400, letterSpacing: "0.04em", color: C.text }}>
                Seus Looks
              </h2>
              <span style={{ fontSize: 12, color: C.muted, fontWeight: 300, letterSpacing: "0.1em" }}>
                — {looks.length} {looks.length === 1 ? "criação" : "criações"}
              </span>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(210px,1fr))", gap: 24 }}>
              {looks.map((look, i) => (
                <LookCard key={look.id} look={look} delay={i * 60}
                  onOpen={() => onOpen(look)} onDelete={() => onDelete(look.id)} />
              ))}
            </div>
          </>
        )}
...
function EmptyState({ onNew }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", paddingTop: 80, textAlign: "center" }}>
      {/* Decorative mannequin outline */}
      <div style={{ width: 80, marginBottom: 32, opacity: 0.18 }}>
        <Mannequin skinHex="#888" bodyId="standard" />
      </div>
      <h3 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 28, fontWeight: 300, color: C.text, letterSpacing: "0.06em" }}>
      ...
      <button className="pill-btn"
        onClick={onNew}
        style={{
          marginTop: 36, padding: "12px 36px", background: C.text, color: C.white,
          fontSize: 11, fontWeight: 400, letterSpacing: "0.15em", textTransform: "uppercase",
          border: `1px solid ${C.text}`, transition: "all 0.25s",
        }}
      >
        Criar Primeiro Look
      </button>
    ...
    >
      ...
      <div style={{ padding: "24px 32px 12px", background: C.nude + "44", position: "relative" }}>
        ...
        <div className="card-overlay"
          style={{
            position: "absolute", inset: 0, background: "rgba(23,19,16,0.72)",
            display: "flex", alignItems: "center", justifyContent: "center",
            opacity: 0, transition: "opacity 0.3s", gap: 12, flexDirection: "column",
          }}
        >
          <button onClick={onOpen}
            style={{
              padding: "9px 22px", background: C.white, color: C.text,
              fontSize: 10, fontWeight: 400, letterSpacing: "0.15em", textTransform: "uppercase",
              border: "none",
            }}
          >
            Editar
          </button>
          <button onClick={(e) => { e.stopPropagation(); if (confirm("Excluir este look?")) onDelete(); }}
            style={{
              padding: "9px 22px", background: "transparent", color: "#FFB4B4",
              fontSize: 10, fontWeight: 400, letterSpacing: "0.15em", textTransform: "uppercase",
      ...
      <div style={{ padding: "16px 20px" }}>
        <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 17, fontWeight: 400, color: C.text, letterSpacing: "0.03em", marginBottom: 4 }}>
          {look.name}
        </div>
...
// ─── Editor Screen ────────────────────────────────────────────────────────────
function EditorScreen({ lookName, onNameChange, mannequin, skinHex, garments, onBack, onSave, onShare, onConfigMannequin, onAddGarment, onRemoveGarment, onGarmentClick }) {
  ...
  return (
    <div className="fadeIn" style={{ minHeight: "100vh", background: C.bg, display: "flex", flexDirection: "column" }}>
      ...
      <header style={{
        ...
        display: "flex", alignItems: "center", justifyContent: "space-between",
        background: C.surface, height: 64,
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
          <button onClick={onBack}
            style={{ fontSize: 11, color: C.muted, fontWeight: 300, letterSpacing: "0.1em", textTransform: "uppercase", display: "flex", alignItems: "center", gap: 6 }}>
            ← Voltar
          </button>
          <div style={{ width: 1, height: 20, background: C.border }} />
          <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 22, fontWeight: 300, letterSpacing: "0.15em", color: C.text }}>
            RHAPSO
          </div>
        </div>

        {/* Look name (inline edit) */}
        <div style={{ flex: 1, display: "flex", justifyContent: "center" }}>
          {editingName ? (
            <input ref={nameRef} value={lookName} onChange={(e) => onNameChange(e.target.value)}
              onBlur={() => setEditingName(false)} onKeyDown={(e) => e.key === "Enter" && setEditingName(false)}
              style={{
                fontFamily: "'Cormorant Garamond',serif", fontSize: 18, fontWeight: 400, letterSpacing: "0.06em",
                color: C.text, background: "transparent", border: "none", borderBottom: `1px solid ${C.border}`,
                textAlign: "center", padding: "2px 8px", minWidth: 180,
              }}
            />
          ) : (
            <button onClick={() => setEditingName(true)}
              style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 18, fontWeight: 400, letterSpacing: "0.06em", color: C.text, display: "flex", alignItems: "center", gap: 8 }}>
              {lookName}
        ...
        <div style={{ display: "flex", gap: 12 }}>
          ...
          <button className="pill-btn"
            onClick={onSave}
            style={{
              padding: "8px 24px", background: C.text, color: C.white,
              fontSize: 10, fontWeight: 400, letterSpacing: "0.14em", textTransform: "uppercase",
              border: `1px solid ${C.text}`, transition: "all 0.25s",
            }}
          >
      ...
      <div style={{ flex: 1, display: "flex", gap: 0 }}>
        ...
        <aside style={{ width: 200, borderRight: `1px solid ${C.border}`, padding: "36px 28px", background: C.surface, display: "flex", flexDirection: "column", gap: 32 }}>
          <div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 14 }}>
              {SKINS.map((s) => (
                <button key={s.id} className="skin-dot"
              ...
              {BODIES.map((b) => (
                <button key={b.id}
                  ...
                  style={{
                    ...
                    fontSize: 11, fontWeight: mannequin.bodyType === b.id ? 500 : 300,
                    letterSpacing: "0.06em", color: mannequin.bodyType === b.id ? C.text : C.muted,
        ...
        <main style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "48px 80px" }}>
          <div style={{ width: "100%", maxWidth: 280, position: "relative" }}>
            <MannequinCanvas
              mannequin={mannequin}
              garments={garments}
              onZoneClick={onAddGarment}
              onGarmentClick={(g) => { if (g.shopUrl) window.open(g.shopUrl, "_blank"); }}
            />
          </div>
        </main>

        {/* ── Right: Garments panel ── */}
        <aside style={{ width: 260, borderLeft: `1px solid ${C.border}`, padding: "36px 28px", background: C.surface, display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          {CATS.map((cat) => {
            const g = garments[cat.id];
            return (
              <div key={cat.id} className="garment-row"
                style={{ borderBottom: `1px solid ${C.border}`, paddingBottom: 16, marginBottom: 8 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: g ? 10 : 0 }}>
                  <span style={{ fontSize: 10, fontWeight: 400, letterSpacing: "0.14em", textTransform: "uppercase", color: C.muted }}>
                    {cat.label}
                  </span>
                  {!g ? (
                    <button onClick={() => onAddGarment(cat.id)}
                      style={{ fontSize: 18, color: C.muted, lineHeight: 1, transition: "color 0.2s" }}
                      title={`Adicionar ${cat.label}`}>
                      +
                    </button>
                  ) : (
                    <button className="remove-btn" onClick={() => onRemoveGarment(cat.id)}
                      style={{ fontSize: 9, color: C.danger, letterSpacing: "0.08em", textTransform: "uppercase", fontWeight: 400 }}>
                      Remover
                    </button>
                  )}
                </div>
                ...
                {g && (
                  <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 12, fontWeight: 400, color: C.text, letterSpacing: "0.02em", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {g.name}
                      </div>
                      {g.shopUrl && (
                        <a href={g.shopUrl} target="_blank" rel="noreferrer"
                          style={{ fontSize: 9, color: C.gold, fontWeight: 400, letterSpacing: "0.1em", textTransform: "uppercase", textDecoration: "none", marginTop: 3, display: "block" }}>
                          Ver na loja ↗
                        </a>
                      )}
                    </div>
                ...
                {!g && (
                  <div style={{ fontSize: 10, color: C.border, fontWeight: 300, letterSpacing: "0.06em", fontStyle: "italic" }}>
                    {cat.hint}
                  </div>
                )}
              </div>
          ...
          <div style={{ marginTop: "auto", paddingTop: 20, borderTop: `1px solid ${C.border}` }}>
            <div style={{ fontSize: 9, color: C.muted, fontWeight: 300, letterSpacing: "0.08em", lineHeight: 1.8, textTransform: "uppercase" }}>
              Clique em uma peça no manequim para acessar o link da loja.
            </div>
          </div>
        </aside>
      </div>
...
// ─── Add Garment Modal ────────────────────────────────────────────────────────
function AddGarmentModal({ category, onAdd, onClose }) {
    ...
    <Overlay onClose={onClose}>
      <div className="slideUp" style={{ background: C.white, width: "100%", maxWidth: 460, padding: "40px 44px" }}>
        ...
        <div style={{ marginBottom: 32 }}>
          <div style={{ fontSize: 9, color: C.muted, fontWeight: 300, letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 6 }}>
            Adicionar peça
          </div>
          <h2 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 24, fontWeight: 400, color: C.text, letterSpacing: "0.04em" }}>
            {cat.label}
          </h2>
        </div>
        ...
        <div style={{ display: "flex", gap: 0, marginBottom: 24, border: `1px solid ${C.border}` }}>
          {["url", "file"].map((m) => (
            <button key={m} onClick={() => setMode(m)}
              style={{
                flex: 1, padding: "9px", fontSize: 10, fontWeight: 400,
        ...
        {mode === "url" ? (
          ...
          <FieldGroup label="Arquivo de imagem">
            <label style={{
              display: "flex", alignItems: "center", justifyContent: "center",
              border: `1px dashed ${C.border}`, padding: "20px", cursor: "pointer",
              fontSize: 11, color: C.muted, letterSpacing: "0.08em", fontWeight: 300,
              transition: "background 0.2s",
            }}>
        ...
        <div style={{ display: "flex", gap: 12, marginTop: 32 }}>
          <button onClick={onClose}
            style={{
              ...
              fontSize: 10, fontWeight: 400, letterSpacing: "0.12em", textTransform: "uppercase", color: C.muted,
            }}>
            Cancelar
          </button>
          <button onClick={handleSubmit}
            disabled={!imageUrl}
            style={{
              flex: 2, padding: "12px", background: imageUrl ? C.text : C.border, color: C.white,
              fontSize: 10, fontWeight: 400, letterSpacing: "0.12em", textTransform: "uppercase",
              cursor: imageUrl ? "pointer" : "not-allowed", transition: "background 0.2s",
            }}>
            Adicionar ao Look
          </button>
        </div>
...
function Toast({ message, onDone }) {
  ...
  return (
    <div style={{
      position: "fixed", bottom: 36, right: 36, zIndex: 9999,
      background: C.text, color: C.white,
      padding: "13px 24px", fontSize: 11, fontWeight: 400, letterSpacing: "0.1em",
...
function Overlay({ children, onClose }) {
  return (
    <div onClick={onClose}
      style={{
        position: "fixed", inset: 0, background: "rgba(23,19,16,0.6)",
        display: "flex", alignItems: "center", justifyContent: "center",
...
export default function Rhapso() {
  ...
  const [looks, setLooks]           = useState(() => stored?.looks || []);
  const [editingLook, setEditingLook] = useState(null);
  const [lookName, setLookName]     = useState("Meu Look");
  const [mannequin, setMannequin]   = useState({ skinTone: "ivory", bodyType: "standard" });
  const [garments, setGarments]     = useState({});
  const [modal, setModal]           = useState(null);     // null | 'addGarment'
  ...
  const openLook = (look) => {
    ...
    setMannequin(look.mannequin || { skinTone: "ivory", bodyType: "standard" });
  ...
  const saveLook = () => {
    const look = {
      id: editingLook?.id || uid(),
      name: lookName || "Look sem nome",
      mannequin,
      garments,
      updatedAt: new Date().toISOString(),
      createdAt: editingLook?.createdAt || new Date().toISOString(),
    };
    setLooks((prev) => {
      const idx = prev.findIndex((l) => l.id === look.id);
      if (idx >= 0) { const next = [...prev]; next[idx] = look; return next; }
      return [look, ...prev];
    });
    setEditingLook(look);
  ...
  return (
    <div>
      ...
      {screen === "home" ? (
        <HomeScreen looks={looks} onNew={newLook} onOpen={openLook} onDelete={deleteLook} />
      ) : (
        <EditorScreen
          lookName={lookName}
          ...
          onSave={saveLook}
          onShare={shareViaClipboard}
          onConfigMannequin={handleConfigMannequin}
          onAddGarment={openAddGarment}
      ...
      {modal === "addGarment" && (
        <AddGarmentModal
          category={addingCat}
          onAdd={(data) => addGarment(addingCat, data)}
